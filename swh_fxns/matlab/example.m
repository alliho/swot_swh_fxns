...
Z = process_swot(swot, fldnm, 'mask',1);
pcolor(X,Y,Z); shading flat;
optstr = [optstr, 'GOOD QA'];
text(lonlims(1)+0.2, latlims(2) - 0.2, optstr, 'FontName','courier', 'FontSize',fs, 'VerticalAlignment','top')
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
thisha = ha(3); axes(thisha); hold on; 
Z = process_swot(swot, fldnm, 'mask',1, 'despike', 1);
pcolor(X,Y,Z); shading flat;
optstr = [optstr, 'de-spiked'];
text(lonlims(1)+0.2, latlims(2) - 0.2,optstr, 'FontName','courier', 'FontSize',fs, 'VerticalAlignment','top')
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
thisha = ha(4); axes(thisha); hold on; 
Z = process_swot(swot, fldnm, 'mask',1, 'despike', 1, 'patch',2);
pcolor(X,Y,Z); shading flat;
...
Z = process_swot(swot, fldnm, 'mask',1, 'despike', 1, 'patch',2, 'correct', 1, 'model', mdl);
pcolor(X,Y,Z); shading flat;
optstr = [optstr, 'empirically corrected'];
text(lonlims(1)+0.2, latlims(2) - 0.2, optstr, 'FontName','courier', 'FontSize',fs, 'VerticalAlignment','top')
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
thisha = ha(6); axes(thisha); hold on; 
Z = process_swot(swot, fldnm, 'mask',1, 'despike', 1, 'patch',2, 'correct', 1, 'model', mdl, 'smooth', 1, 'Lavg',5);
pcolor(X,Y,Z); shading flat;
...
savejpg(gcf, ['example_processing_' datestr(swot.t0, 'yyyymmddTHHMMSS')], savepath, saveopt)